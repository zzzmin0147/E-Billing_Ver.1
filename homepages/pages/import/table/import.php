...
if (isset($_POST['import'])) {
    ...
    if ($_FILES['excel_file']['name'] != "IMBook4.csv") {

        echo "<script>";
        echo "alert('ชื่อไฟล์ไม่ใช่ IMBook4');";
        ...
        while (count($foo) > $count) {
            $invSo = $foo[$count][1];
            $check = " SELECT invSo FROM [SNC_Webapp].[dbo].[sonicdb] WHERE invSo = ? ";
        ...
        if ($count_inv > 0) {
            ...
            while (count($foo) > $count_s) {
                ...
                $advanceOtherDescription = $foo[$count_s][20];
                ...
                $chargeOtherDescription = $foo[$count_s][38];
